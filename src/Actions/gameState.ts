...
export const finishGame = createAsyncThunk('game/finishGame', async (_, { getState, dispatch }) => {
...
export const rebet = createAsyncThunk('game/rebet', async (_, { getState, dispatch }) => {
...
const playerWon = createAsyncThunk('game/playerWon', async (_, { getState, dispatch }) => {
  const state = getState() as RootState;
  const bet = state.table.currentBet;
...
const playerLost = createAsyncThunk('game/playerWon', async (_, { getState, dispatch }) => {
  const state = getState() as RootState;
  const bet = state.table.currentBet;
  const balance = state.player.balance;
  dispatch(setBalance(balance - bet));
